...
def get_negative_samples(
    ...
    """Download CIFAR-10 and create a PyTorch Dataset with n_samples random images.

    Args:
        n_samples (int): Number of random negative samples to extract. Default is 3000.
        seed (int, optional): Random seed for reproducible sampling. Default is 42.
        train (bool, optional): Use CIFAR-10 train set (True) or held-out test set (False). Default is True.

    Returns:
        NegativeDataset: PyTorch Dataset yielding (tensor of shape (4, 224, 224), label=7, 0.0, False).
    """
    cifar_dataset = datasets.CIFAR10(root="./data/negatives", download=True, train=train)
